...
export default function BatteryGauge() {
  ...
  // Treat small flows as idle: the inverter draws ~30-80 W of parasitic
  // power even when discharge is paused, so anything under this threshold
  // is standby noise rather than a real charge/discharge to the house.
  const FLOW_THRESHOLD_W = 100;
  const isCharging =
    power.Flows.Grid_to_Battery > FLOW_THRESHOLD_W ||
    power.Flows.Solar_to_Battery > FLOW_THRESHOLD_W;
  const isDischarging =
    power.Flows.Battery_to_House > FLOW_THRESHOLD_W ||
    power.Flows.Battery_to_Grid > FLOW_THRESHOLD_W;
